<template>
	<view class="wrap u-rela">
					...
					<view class="u-p-20">
						...
						<view class="base-info ">
							...
							<view class="goods-detail">
								...
								<text class="number">库存:{{goods.stock}}</text>
								<text class="sales">销量:{{goods.sales}}</text>
							</view>
						</view>
						<!-- tabs组件 -->
						<u-tabs :list="tabsList" :scrollable="false" :current="tabsCurrent" @change="tabsChange"></u-tabs>
				...
				<view class="navigation">
					<view class="left">
						<view class="item" @tap="collect">
							<u-icon v-if="!isCollect" name="heart" :size="40" :color="$u.color['contentColor']"></u-icon>
		...
		<template v-else>
			<view class="loading">
				<u-loading-page size="40" color="#2979ff"></u-loading-page>
				<view>
...
<script>
	export default {
		...
		methods: {
			// 获取商品信息接口
			async getBaseInfo(){
				const res = await this.$API.reqGetGoodsDetail(this.goodsId)
				console.log(res);
				// 商品基本信息
			...
			async collectNot(){
				if(!this.$u.utils.isLogin()){}// 如果没有登录先去登录
					...
					// 请求收藏接口
					await this.$API.reqIsCollect(this.goodsId)
				}
			},
			// 添加购物车(自带节流效果)
			async addCart(){
				if(!this.$u.utils.isLogin()){}// 如果没有登录先去登录
					...
					// 发送请求
					const res= await this.$API.reqAddCart(params)
				}
			},
			async cartLen(){
				// 如果登录成功在调用这个接口
				if(this.user.user_token){
					// 获取购物车商品数量
					const res=await this.$API.reqCartList()
					// 累加总的商品数量
